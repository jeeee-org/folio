...
| 親へ | 一覧の上端のパスの行をクリック |
| スクロール | ホイール。ポインタが一覧の上なら一覧、プレビューの上ならプレビュー、全画面では本文が動く |

`folio browse`の間はfolioがマウスを受け取るので、画面の文字を選んでコピーするには**`Shift`を押しながらドラッグ**する（Windows Terminal）。`folio view`を単体で開いた時はマウスを受け取らないので、そのままドラッグ（`Alt`+ドラッグで矩形）で選べる。
