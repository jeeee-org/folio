...
vim風の画面の上に問題（「3単語先の頭へ」「対応する括弧へ」「『render』を検索して移動」…）が出る。既定は**厳格**で、模範解答（`5G`と`5gg`のような同等の別解を含む）と違うキーを押した時点で不正解になり、打ったキーと答えが出る。`--loose`にすると違うキーを押しても続けられ、制限時間内にカーソルが目標に着けばクリア。クリア100点、模範解答より多い打鍵1つにつき10点減、不正解・時間切れ0点。出題は16種で、`h j k l`・`w b e`・`f t ; ,`・`0 ^ $`・`gg G`・`{ }`・`%`・`H M L`・`Ctrl-d/u/f/b`・`/ ? n N`・回数指定が使える。結果は`~/.local/share/folio/practice.jsonl`に残り、成績の悪い型が多めに出るようになる。`Ctrl-c`で中断（記録しない）。
