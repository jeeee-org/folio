...
`j` `k` `gg` `G` `d` `u`で一覧を移動、`h`で親へ、`l`/`Enter`でディレクトリに入る。ファイルで`Enter`を押すとビューアーが全画面で開き、`q`で一覧に戻る。`e`でvim、`.`で隠しファイルの表示切替、`Ctrl-d`/`Ctrl-u`でプレビューをスクロール、`q`で終了。1MBを超えるファイルとUTF-8でないファイルはプレビューしない。ファイル操作（コピー・移動・削除）はまだ無く、yaziと併用する前提。

シェルで「終了したディレクトリに移動する」には、yaziと同じ方式で`--cwd-file`を使う:
...
P1（ビューアー・yazi連携・vim往復）、P2（目次・検索・表・設定）、P3（テキスト・ソースコード・CSV/TSV）、P4（vimの移動の練習モード）、P5の第一歩（`folio browse`: 一覧・移動・プレビュー）まで完了。次はファイル操作かyaziの置き換えの判断。
